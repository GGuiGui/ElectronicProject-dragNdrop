...
try {
  ...
  logger.error(
    `Failed to load config file from ${configPath}. Using default.`,
    error
  );
...
if (fs.existsSync(iconName)) {
  ...
  https
    .get("https://img.icons8.com/ios/452/drag-and-drop.png", (response) => {
...
ipcMain.on("ondragstart", (event, filePath) => {
  ...
  logger.info(`Drag origin filePath: ${filePath}`);

  filePath = replaceOSfilePath(filePath);

  logger.info(`Drag trans filePath: ${filePath}`);
